...
private:
    map<string, map<int,double>> word_to_documents_freqs;
    ...
    double ComputeIDF(const string& word) const{
        return log(document_count_*1.0/word_to_documents_freqs.at(word).size());
    }
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        for (const string& word : query.plus_words) {
            if (word_to_documents_freqs.count(word) == 0) {
                continue;
            }
            const double idf=ComputeIDF(word);
            for (const auto [document_id,tf] : word_to_documents_freqs.at(word)) {
